...
return new class extends Migration
{
    ...
    {
        Schema::create('tr_acc_yearly_closing', function (Blueprint $table) {
            // Primary Key
            $table->id();
            ...
            // Opening Balance (saldo awal tahun)
            $table->decimal('opening_debet', 18, 2)->default(0)->comment('Saldo awal tahun debet');
            $table->decimal('opening_kredit', 18, 2)->default(0)->comment('Saldo awal tahun kredit');
            $table->decimal('opening_balance', 18, 2)->default(0)->comment('Saldo awal tahun netto');
            
            // Mutasi Tahun Ini (aggregate dari 12 monthly closing)
            $table->decimal('mutasi_debet', 18, 2)->default(0)->comment('Total debet tahun ini');
            $table->decimal('mutasi_kredit', 18, 2)->default(0)->comment('Total kredit tahun ini');
            $table->decimal('mutasi_netto', 18, 2)->default(0)->comment('Netto mutasi tahun ini');
            $table->integer('jumlah_transaksi')->default(0)->comment('Total transaksi dalam tahun');
            
            // Closing Balance
            $table->decimal('closing_debet', 18, 2)->default(0)->comment('Saldo akhir tahun debet');
            $table->decimal('closing_kredit', 18, 2)->default(0)->comment('Saldo akhir tahun kredit');
            $table->decimal('closing_balance', 18, 2)->default(0)->comment('Saldo akhir tahun netto');
            
            // Monthly Breakdown (untuk trace back)
            $table->json('monthly_summary')->nullable()->comment('Summary per bulan (Jan-Dec)');
        ...
        DB::statement("ALTER TABLE tr_acc_yearly_closing ALTER COLUMN coa_code VARCHAR(50) NOT NULL");
        
        // Add Indexes after altering column type
        Schema::table('tr_acc_yearly_closing', function (Blueprint $table) {
            $table->index(['closing_year', 'coa_code', 'version_number'], 'idx_year_coa_version');
            $table->index(['version_status'], 'idx_version_status');
            $table->index(['closing_year'], 'idx_year');
        ...
        DB::statement("EXEC sp_addextendedproperty 
            ...
            @value = N'Tabel closing tahunan dengan version control untuk rekap transaksi per COA', 
            @level0type = N'SCHEMA', @level0name = 'dbo',
            @level1type = N'TABLE', @level1name = 'tr_acc_yearly_closing'");
    }
    ...
    {
        Schema::dropIfExists('tr_acc_yearly_closing');
    }
};
