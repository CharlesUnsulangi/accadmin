# Stored Procedure: SP_compare_tr_acc_total_audit

## Deskripsi
SP ini digunakan untuk membandingkan dua versi audit total pada tabel `tr_acc_total_audit` untuk analisis perubahan saldo dan hasil audit.
...
## Logika Dasar
1. Ambil data audit total untuk dua versi yang dibandingkan.
2. Hitung selisih saldo dan hasil audit per COA.
3. Return hasil perbandingan untuk reporting/audit.

## Contoh SQL Template
```sql
CREATE PROCEDURE SP_compare_tr_acc_total_audit
    @audit_period VARCHAR(10),
    @version_number_1 INT,
    @version_number_2 INT
AS
BEGIN
    ...
    FROM tr_acc_total_audit a
    INNER JOIN tr_acc_total_audit b
        ON a.audit_period = b.audit_period
        AND a.coa_main_id = b.coa_main_id
        AND a.coasub1_id = b.coasub1_id
        AND a.coasub2_id = b.coasub2_id
        AND a.coa_id = b.coa_id
    WHERE a.audit_period = @audit_period
