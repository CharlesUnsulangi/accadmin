    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-lg shadow-lg p-4">
            <div class="text-white">
                ...
                <p class="text-3xl font-bold mt-1">{{ $coaSub2s->total() }}</p>
            </div>
        </div>
        <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-lg shadow-lg p-4">
            <div class="text-white">
                ...
                <p class="text-3xl font-bold mt-1">{{ $mains->count() }}</p>
            </div>
        </div>
        <div class="bg-gradient-to-br from-purple-500 to-purple-600 rounded-lg shadow-lg p-4">
            <div class="text-white">
                ...
                <p class="text-3xl font-bold mt-1">{{ $sub1s->count() }}</p>
            </div>
        </div>
        <div class="bg-gradient-to-br from-orange-500 to-orange-600 rounded-lg shadow-lg p-4">
            <div class="text-white">
                ...
                <p class="text-3xl font-bold mt-1">{{ $coaSub2s->count() }}</p>
            </div>
        </div>
    </div>
                    ...
                    @forelse($coaSub2s as $sub2)
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-mono font-semibold text-gray-900">{{ $sub2->coasub2_code }}</div>
                                <div class="text-xs text-gray-500">ID: {{ $sub2->coasub2_id ?? '-' }}</div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-xs space-y-1">
                                    @if($sub2->coaSub1 && $sub2->coaSub1->coaMain)
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span class="text-gray-700">{{ $sub2->coaSub1->coaMain->coa_main_desc }}</span>
                                        </div>
                                    @endif
                                    @if($sub2->coaSub1)
                                        <div class="flex items-center gap-2 ml-4">
                                            <span class="px-2 py-0.5 bg-green-100 text-green-800 rounded text-xs font-medium">L2</span>
                                            <span class="text-gray-700">{{ $sub2->coaSub1->coasub1_desc }}</span>
                                        </div>
                                    @endif
                                    <div class="flex items-center gap-2 ml-8">
                                        <span class="px-2 py-0.5 bg-purple-100 text-purple-800 rounded text-xs font-medium">L3</span>
                                        <span class="text-gray-700 font-semibold">{{ $sub2->coasub2_desc }}</span>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-900 font-medium">{{ $sub2->coasub2_desc }}</div>
                                <div class="text-xs text-gray-500 mt-1">Code: {{ $sub2->coasub2_code }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900">
                                    <a href="{{ route('coa.modern') }}?filter_sub2={{ $sub2->coasub2_code }}" 
                                       ...
                                       title="Click to view {{ $sub2->coas->count() }} detail accounts">
                                        ...
                                        {{ $sub2->coas->count() }} accounts
                                    </a>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                    {{ $sub2->rec_status === '1' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
    ...
    <div class="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
        <div class="flex items-start">
            ...
            <div class="text-sm text-blue-800">
                ...
                <p class="mt-2">For the new flexible system (H1-H6 in same table), use <a href="{{ route('coa.modern') }}" class="underline font-semibold">COA Modern</a></p>
            </div>
        </div>
    </div>
</div>
